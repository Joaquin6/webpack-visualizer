...
function getPartition(data, chartSize) {
...
export default function createVisualization({ svgElement, root, onHover, onUnhover }) {
	...
	let partition = getPartition(root, chartSize)
	...
	let nodes = partition.descendants().filter((d) => d.dx > 0.005) // 0.005 radians
	...
	let topPadding =
		...
		window.scrollY -
		(d3.select(chart)._groups[0][0].getBoundingClientRect().top + window.scrollY)
